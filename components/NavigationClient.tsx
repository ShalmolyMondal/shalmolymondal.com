...
function NavigationPendingIndicator() {
    ...
    return (
        <span className="pointer-events-none absolute right-1.5 top-1/2 inline-flex h-3 w-3 -translate-y-1/2 items-center justify-center" aria-live="polite">
            {pending && (
                <>
                    <span className="h-2.5 w-2.5 animate-spin rounded-full border border-[#818CF8]/35 border-t-[#C7D2FE]" aria-hidden="true" />
    ...
    return (
        <nav
            ...
            className={`fixed top-0 left-0 right-0 z-50 transition-[background-color,border-color,box-shadow] duration-300 ${solidNav
                    ? 'bg-[#0B0C14]/98 border-b border-[#6366F1]/15 shadow-lg shadow-black/20'
                    : 'bg-[#0B0C14]/95 border-b border-[#6366F1]/10 lg:bg-transparent lg:border-transparent lg:shadow-none'
                }`}
        >
                ...
                <div className="flex items-center justify-between relative">
                    ...
                    <div className="hidden lg:flex items-center">
                        <Link
                            href={cta.href}
                            prefetch
                            className="relative flex items-center justify-center px-5 py-2 bg-gradient-to-r from-[#6366F1] to-[#4F46E5] text-white text-sm font-medium rounded-lg hover:from-[#818CF8] hover:to-[#6366F1] transition-[transform,box-shadow] duration-200 shadow-lg shadow-[#6366F1]/20 hover:shadow-[#6366F1]/40 hover:scale-105"
                        >
                            {cta.name}
                            <NavigationPendingIndicator />
                        </Link>
                ...
                {mobileMenuOpen && (
                    <div id="mobile-navigation" className="animate-mobile-menu lg:hidden mt-3 pb-3 border-t border-[#6366F1]/10 pt-3 space-y-2">
                        ...
                        <Link
                            ...
                            className="relative flex w-full items-center justify-center px-4 py-2.5 bg-gradient-to-r from-[#6366F1] to-[#4F46E5] text-white text-sm font-medium rounded-lg hover:from-[#818CF8] hover:to-[#6366F1] transition-colors text-center mt-4"
                        >
                            {cta.name}
                            <NavigationPendingIndicator />
                        </Link>
