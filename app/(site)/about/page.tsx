                ...
                <div className="max-w-3xl mx-auto px-6">
                            ...
                            <div className="flex-1 w-full">
                                <div className="flex items-start justify-between mb-4">
                                    <div>
                                        ...
                                        <p className="text-[#939DB8] text-base">{content.profileSubtitle}</p>
                                    </div>
                                    <div className="flex items-center gap-3">
                                        ...
                                        <a href={personal.social.instagram} target="_blank" rel="noopener noreferrer" className="text-[#939DB8] hover:text-white transition-colors">
                    ...
                    <section className="mb-16">
                        <h2 className="text-2xl font-bold mb-5">
                            <span className="bg-gradient-to-r from-[#C9D3EE] to-[#818CF8] bg-clip-text text-transparent">{content.aboutTitle}</span>
                        </h2>
                    ...
                    <section className="mb-16">
                        <h2 className="text-2xl font-bold mb-2">
                            <span className="bg-gradient-to-r from-[#C9D3EE] to-[#818CF8] bg-clip-text text-transparent">{content.smallThingsTitle}</span>
                        </h2>
                        <p className="text-sm text-[#939DB8] mb-6">{content.smallThingsDescription}</p>

                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                            {content.smallThings.map((item) => (
                                <div key={`${item.icon}-${item.label}`} className="flex flex-col items-center text-center py-8 px-4 rounded-2xl bg-[#171926] border border-[#727DA1]/10 hover:border-[#6366F1]/40 hover:bg-[#1d1f33] hover:scale-105 hover:shadow-lg hover:shadow-[#6366F1]/10 transition-all duration-300">
                                    <SmallThingIcon icon={item.icon} />
                                    <p className="text-sm font-medium leading-snug text-[#C9D3EE]">{item.label}</p>
                                </div>
                            ))}
                        </div>
                    </section>
                    ...
                    <section className="mb-16">
                        <h2 className="text-2xl font-bold mb-2">
                            <span className="bg-gradient-to-r from-[#C9D3EE] to-[#818CF8] bg-clip-text text-transparent">{content.currentlyTitle}</span>
                        </h2>
                        <p className="text-sm text-[#939DB8] mb-6">{content.currentlyDescription}</p>

                        <div className="space-y-4">
                            {content.currently.map((item) => {
                                const Icon = currentIconMap[item.icon];
                                return (
                                    <div key={`${item.icon}-${item.label}`} className="flex items-center gap-4 p-4 rounded-xl bg-[#171926]/50 border-l-2 border-[#6366F1]/30 hover:scale-[1.02] hover:bg-[#171926] transition-all duration-200">
                                        <Icon className="w-5 h-5 text-[#818CF8] flex-shrink-0" strokeWidth={1.5} />
                                        <div>
